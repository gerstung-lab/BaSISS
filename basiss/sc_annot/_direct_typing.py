...
def iss_annotation(sample, broad_markers, narrow_markers, condition, th_dist = 5, pix2um=0.325):
    ...
    expression_table = iss_spots.groupby(['Gene','assigned_nucl']).size().unstack('assigned_nucl', fill_value=0).T.iloc[1:,:]

    res_low = cell_type_decision(expression_table, broad_markers)
    res_high = cell_type_decision(expression_table, narrow_markers, condition=inverse_dict(condition), previous_annots=res_low)
