...
def mask_infisble(mut_sample_list, scale, probability=0.6, plot=False):
    """Create a mask of infisible areas according to a set of empirical rules
...
def generate_data4model(samples_list, genes, M, n_aug=1, unified_names=None):
    ...
    unified_names : list
        Map of sample names -> standard names for each sample
    ...
    if not unified_names is None:
        iss_data = [
            np.transpose(np.array([samples_list[i].gene_grid[unified_name1[i][k]] for k in genes]), [1, 2, 0]).reshape(
                -1, n_genes) for i in range(n_samples)]
    else:
        iss_data = [
            np.transpose(np.array([samples_list[i].gene_grid[k] for k in genes]), [1, 2, 0]).reshape(-1, n_genes) for i
            in range(n_samples)]
...
def to_grid(x_raw, y_raw, grid_params, img_size):
    """Data binning to a predefined grid
