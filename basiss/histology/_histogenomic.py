...
class Histogenomic_associations:
    '''
    We store histological annotations, phylogenetic composition and extra infomration
    '''
    def __init__(self,
    ...
    def subset_ids(self,
        ...
        list_ids = [list(self.histology_df.index.astype(str)[hist_condition]), list(np.array(list(self.composition_dict.keys()))[comp_condition])]
        if self.extra_dfs is not None:
            for extra_name in self.extra_dfs_names:
    ...
    def filter_by_clone(self, clone_id, th = -1):
        ...
        if type(clone_id) == list:
            for item in self.common_ids:
                ...
                if self.composition_dict[item].mean(axis=0)[clone_id].sum() / self.composition_dict[item].mean(axis=0).sum()  > th and logic:
                    valid_ids.append(True)
                else:
                    valid_ids.append(False)
        else:
            for item in self.common_ids:
                if self.composition_dict[item].mean(axis=0)[clone_id] / self.composition_dict[item].mean(axis=0).sum()  > th and self.composition_dict[item].mean(axis=0)[:-1].argmax() == clone_id:
                    valid_ids.append(True)
                else:
                    valid_ids.append(False)
        return valid_ids
    ...
    def sort_ids(self, th=1):
        compos_list = [self.composition_dict[i].mean(axis=0) / self.composition_dict[i].mean(axis=0).sum() for i in self.common_ids]
        sorting_ids = np.argsort(np.array(list(map(lambda x: (~(x[:-1].sum() > th), np.argmax(x[:-1]), -np.max(x[:-1])), compos_list)) + [None], dtype=tuple)[:-1])
        #sorting_ids = np.argsort(np.array(list(map(lambda x: (np.argmax(x), -np.max(x)), compos_list)) + [None], dtype=tuple)[:-1])
