...
def output_ducts_composition_raw(mut_sample_list,
    ...
    gfields_d = np.concatenate([gfields_d[:, :, :, :(n_subcl_dim - 3)],
                                gfields_d[:, :, :, (n_subcl_dim - 3):(n_subcl_dim - 1)].sum(axis=3)[:, :, :, None]],
                               axis=3)  # collapse wt, remove residuals
...
def output_cell_composition_raw(sample_panel_list: list,
    ...
    for sample_panel_i, sample_panel in enumerate(sample_panel_list):
        ...
        if mode == 'inner':
            for i in range(len(paths)):
                poly_area.append(
                    np.round(np.sum([shoelace(x) for x in expand_poly(paths[i] * pixel2um, distance=distance_inside)])))
            ...
            for i in range(len(paths)):
                poly_area.append(np.round(
                    np.sum([shoelace(x) for x in expand_poly(paths[i] * pixel2um, distance=distance_outside)]) -
        ...
        for expression_type in expression_types:
            expression_numbers[sample_panel_i][expression_type] = pd.DataFrame(
                np.stack(expression_numbers[sample_panel_i][expression_type]),
                index=paths_id,
