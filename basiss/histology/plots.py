...
def plot_by_epith_extra(hga_selected,
    ...
    for i in range(len(epith_names)):
        hga_selected.subset_ids(hist_condition=np.isin(hga_selected.histology_df['Epithelial cells'], epith_names[i]),
                                comp_condition=hga_selected.exist_comp_condition())
    ...
    axs[0, 0].imshow(
        subcl_comp2color(np.concatenate(dat_matrix, axis=1)[:-1], colornames=colornames, colorscale=colorscale)[reorder,
        :], vmin=0, vmax=1)
    ...
    if hist_columns is not None:
        ...
        if flag_categorical:
            axs[2, 0].imshow(categ_df2color(np.concatenate(hist_dat, axis=1).astype(int), colours=cat_colours), vmin=0,
                             vmax=1)
        else:
    ...
    for i in range(len(sbcl_names)):
        inset_ax = inset_axes(axs[0, 1],
                              ...
                              bbox_transform=axs[
                                  0, 1].transAxes)  # center, you can check the different codes in plt.legend?
    ...
    inset_ax = inset_axes(axs[1, 1],
                          ...
                          bbox_transform=axs[
                              1, 1].transAxes)  # center, you can check the different codes in plt.legend?
            ...
            for i in range(len(hist_columns)):
                ...
                for j in range(len(categories)):
                    inset_ax = inset_axes(axs[2, 1],
                                          ...
                                          loc=10,
                                          bbox_to_anchor=(
                                          0.1 + 1 / len(categories) * j, 1 - 1 / len(hist_columns) * (i + 1) - 0.1,
                                          1 / len(categories), 1 / len(hist_columns)),
                                          bbox_transform=axs[
                                              2, 1].transAxes)  # center, you can check the different codes in plt.legend?

                    inset_ax.scatter(0, 0, marker='s', s=100,
                                     color=categ_df2color(np.array([[j]]), colours=cat_colours))
                    inset_ax.axis('off')
...
def plot_dcis_histogenomics(dcis_data,
    ...
    for i, sample_name in enumerate(plotnames):
        ...
        axs[0, i].imshow(subcl_comp2color(proportions, colornames=np.array(
            ['grey', 'green', 'mediumorchid', 'dodgerblue', 'orangered',
    ...
    for i in range(6):
        inset_ax = inset_axes(axs[0, 2],
                              ...
                              bbox_transform=axs[
                                  0, 2].transAxes)  # center, you can check the different codes in plt.legend?
    ...
    inset_ax = inset_axes(axs[1, 2],
                          ...
                          bbox_transform=axs[
                              1, 2].transAxes)  # center, you can check the different codes in plt.legend?
    ...
    for i in range(2):
        ...
        for j in range(len(categories)):
            inset_ax = inset_axes(axs[1, 2],
                                  ...
                                  loc=10,
                                  bbox_to_anchor=(
                                  0.1 + 1 / len(categories) * j, 1 - 1 / 4 * (i + 1 + 2) - 0.1, 1 / len(categories),
                                  1 / 4),
                                  bbox_transform=axs[
                                      1, 2].transAxes)  # center, you can check the different codes in plt.legend?
    ...
    for i in range(len(selected_columns)):
        ...
        for j in range(len(categories)):
            inset_ax = inset_axes(axs[2, 2],
                                  ...
                                  loc=10,
                                  bbox_to_anchor=(
                                  0.1 + 1 / len(categories) * j, 1 - 1 / len(selected_columns) * (i + 1) - 0.1,
                                  1 / len(categories), 1 / len(selected_columns)),
                                  bbox_transform=axs[
                                      2, 2].transAxes)  # center, you can check the different codes in plt.legend?
...
def vulcano_plot_glmm(diff_data, signal_data, colors,
    ...
    for xyzf in zip(delta.mean(0), p, genes, fdr_val):
        ...
        ys.append(xyzf[1])
        if xyzf[3] < fdr_val_cutoff and not (
                np.exp(xyzf[0]) < fold_change_cutoff and np.exp(xyzf[0]) > 1 / fold_change_cutoff):
...
def plot_cell_composition(data,
    ...
    probs = np.concatenate(
        [np.exp(np.array(beta)) / np.exp(np.array(beta)).sum(2)[:, :, None] for beta in composition_estimates], axis=2)
    ...
    if n_entities == 3:
        ...
        raise ValueError(
            'Only 2 and 3 group comparison is implemented, I was too tired to think how to nicely spread violin groups')

    for i in range(n_entities):
        ps = probs[:, i, subset]
        vp = plt.violinplot([ps[:, sub_gene][(ps[:, sub_gene] < np.percentile(ps[:, sub_gene], 99)) & (
                    ps[:, sub_gene] > np.percentile(ps[:, sub_gene], 1))]
                             for sub_gene in range(len(subset))], positions=np.arange(len(subset)) + move[i],
                            showextrema=False, widths=0.3)
        for j in range(len(subset)):
            jit = np.random.randn(data_probs[r2c == i].shape[0])
            plt.scatter(np.array([j] * data_probs[r2c == i].shape[0]) + move[i] + jit / 50,
                        data_probs[r2c == i][:, subset[j]], color='black', s=5)
    ...
    for j in range(len(subset)):
        ...
        for i, ps in enumerate(pairwise_comparisons):
            ...
            if fdr_values[i, j] < 0.1:
                boxplot_annotate_brackets(num1, num2, j + np.array(move),
                                          [np.max(data_probs[r2c == x][:, subset[j]]) for x in range(n_entities)],
                                          str(np.round(fdr_values[i, j], 5)), dh=0.01 + dh_extra)
                dh_extra += 0.1
    ...
    probs_combined = np.concatenate([probs[:, :, [celltype2position[x] for x in
                                                  ['B-cells', 'T-cells', 'Myeloid', 'Immune broad']]].sum(2)[:, :,
                                     None],
                                     probs[:, :, celltype2position['Fibroblasts + PVL']][:, :, None],
                                     probs[:, :, celltype2position['Epithelial broad']][:, :, None]], axis=2)

    data_probs_combined = np.concatenate([data_probs[:, [celltype2position[x] for x in
                                                         ['B-cells', 'T-cells', 'Myeloid', 'Immune broad']]].sum(1)[:,
                                          None],
    ...
    for i in range(n_entities):
        ps = probs_combined[:, i, :]
        vp = plt.violinplot([ps[:, sub_gene][(ps[:, sub_gene] < np.percentile(ps[:, sub_gene], 99)) & (
                    ps[:, sub_gene] > np.percentile(ps[:, sub_gene], 1))] for sub_gene in range(3)],
        ...
        for j in range(3):
            ...
            plt.scatter(np.array([j] * data_probs_combined[r2c == i].shape[0]) + move[i] + jit / 50,
                        data_probs_combined[r2c == i][:, j], color='black', s=5)
    ...
    for j in range(len(subset)):
        ...
        for i, ps in enumerate(pairwise_comparisons):
            ...
            if fdr_values[i, j] < 0.1:
                boxplot_annotate_brackets(num1, num2, j + np.array(move),
                                          [np.max(data_probs_combined[r2c == x][:, j]) for x in range(n_entities)],
                                          str(np.round(fdr_values[i, j], 5)), dh=0.01 + dh_extra)
                dh_extra += 0.1
