...
def signif_comparisons(diff_data, signal_data, comparisons, fdr_th=0.1, pre_test_cut=0.4):
    ...
    for comp in comparisons:    
        ...
        mask = (signal_data_sub['value'].sum(level=0)/signal_data_sub['n_nucl'].sum(level=0) * signal_data_sub['n_nucl'].mean(level=0)) > pre_test_cut
        ...
        f = f[fdr_val < fdr_th]
        #signif_gene_ids += list(np.where(fdr_val < 0.1)[0])
